...
class SocialApp(models.Model):
    ...
    provider = models.CharField(verbose_name=_('اداره کننده'),
                                max_length=30,
                                choices=providers.registry.as_choices())
    name = models.CharField(verbose_name=_('نام'),
                            max_length=40)
    client_id = models.CharField(verbose_name=_('مشخصات مشتری'),
                                 max_length=191,
                                 help_text=_('App ID, or consumer key'))
    secret = models.CharField(verbose_name=_('کد محرمانه'),
                              max_length=191,
                              help_text=_('API secret, client secret, or'
                              ' consumer secret'))
    key = models.CharField(verbose_name=_('کد'),
                           max_length=191,
                           blank=True,
                           help_text=_('Key'))
...
class SocialAccount(models.Model):
    ...
    uid = models.CharField(verbose_name=_('مشخصات کاربر'),
                           max_length=app_settings.UID_MAX_LENGTH)
    last_login = models.DateTimeField(verbose_name=_('آخرین ورود'),
                                      auto_now=True)
    date_joined = models.DateTimeField(verbose_name=_('تاریخ اضافه شدن'),
                                       auto_now_add=True)
    extra_data = JSONField(verbose_name=_('اطالاعات اضافی'), default=dict)

    class Meta:
        unique_together = ('provider', 'uid')
        verbose_name = _('حساب شبکه ی اجتماعی')
        verbose_name_plural = _('حساب شبکه های اجتماعی')
...
class SocialToken(models.Model):
    ...
    class Meta:
        ...
        verbose_name = _('توکن شبکه ی اجتماعی')
        verbose_name_plural = _('توکن های شبکه های اجتماعی')
